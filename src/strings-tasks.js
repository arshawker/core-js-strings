...
function repeatString(str, times) {
  return times < 0 || !Number.isInteger(times) ? '' : str.repeat(times);
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (/[aeouiyAEOUIY]/.test(str[i])) count += 1;
  }
  return count;
}
...
function extractEmails(str) {
  return str.split(/\s*;\s*/).map((email) => email.trim());
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-zA-Z]/g, (char) => {
    ...
    const offsetCharCode = ((char.charCodeAt() - base + offset) % 26) + base;
